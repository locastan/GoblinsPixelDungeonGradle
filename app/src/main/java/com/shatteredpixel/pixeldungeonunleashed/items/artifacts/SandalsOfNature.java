...
public class SandalsOfNature extends Artifact {
	...
	protected String inventoryTitle = "Select seed:";
	...
	public ArrayList<String> actions( Hero hero ) {
		...
		if (isEquipped( hero ) && level < 3 && !cursed)
			actions.add(AC_FEED);
		if (isEquipped( hero ) && charge > 0 && level > 0)
			actions.add(AC_ROOT);
		return actions;
	}
	...
	public String desc() {
		...
		if (!seeds.isEmpty()){
			...
			for (int i = 0; i < seedsArray.length-1; i++)
				desc += " " + (!seedsArray[i].equals("spores of Yumyuck Moss") ? seedsArray[i].substring(8) : seedsArray[i].substring(10)) + ",";

			desc += " " + (!seedsArray[seedsArray.length-1].equals("spores of Yumyuck Moss") ? seedsArray[seedsArray.length-1].substring(8) : seedsArray[seedsArray.length-1].substring(10)) + ".";
		}
